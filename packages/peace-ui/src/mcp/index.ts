  ...
  return {
    tools: [
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            registries: z
              .array(z.string())
              .describe(
                "Array of registry names to search (e.g., ['@peace', '@acme'])"
              ),
            limit: z
              .number()
              .optional()
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of item names with registry prefix (e.g., ['@peace/button', '@peace/card'])"
              ),
          })
        ),
      },
      ...
      },
      {
        ...
        description:
          "Get the peace-ui CLI add command for specific items in a registry. This is useful for adding one or more components to your project.",
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of items to get the add command for prefixed with the registry name (e.g., ['@peace/button', '@peace/card'])"
              ),
          })
        ),
      },
          ...
          content: [
            {
              ...
              text: dedent`The following registries are configured in the current project:
                ...
                \`${await npxPeaceUi("view @name-of-registry")}\`

                For example: \`${await npxPeaceUi(
                  "view @peace"
                )}\` or \`${await npxPeaceUi(
                "view @peace @acme"
            ...
            content: [
              {
                ...
                Make sure the item names are correct and include the registry prefix (e.g., @peace/button).`,
              },
            ],
          }
        }
      ...
      case "get_add_command_for_items": {
        ...
        return {
          content: [
            {
              type: "text",
              text: await npxPeaceUi(`add ${args.items.join(" ")}`),
            },
          ],
        }
      }
