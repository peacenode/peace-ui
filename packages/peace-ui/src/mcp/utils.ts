...
const PEACE_UI_CLI_COMMAND = "peace-ui@latest"

export async function npxPeaceUi(command: string) {
  const packageRunner = await getPackageRunner(process.cwd())
  return `${packageRunner} ${PEACE_UI_CLI_COMMAND} ${command}`
}
...
export function formatSearchResultsWithPagination(
  ...
  const formattedItems = results.items.map((item) => {
    ...
    parts.push(
      `\n  Add command: \`${npxPeaceUi(`add ${item.addCommandArgument}`)}\``
    )
