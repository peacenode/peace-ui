...
export class RegistryParseError extends RegistryError {
  ...
  constructor(public readonly item: string, parseError: unknown) {
    ...
    super(message, {
      ...
      context: { item },
      suggestion:
        "The registry item may be corrupted or have an invalid format. Please make sure it returns a valid JSON object. See https://peace-ui.dev/schema/registry-item.json.",
    })
...
export class RegistryInvalidNamespaceError extends RegistryError {
  constructor(public readonly name: string) {
    const message = `Invalid registry namespace: "${name}". Registry names must start with @ (e.g., @peace, @v0).`
...
export class ConfigMissingError extends RegistryError {
  constructor(public readonly cwd: string) {
    ...
    super(message, {
      ...
      context: { cwd },
      suggestion:
        "Run 'npx peace-ui@latest init' to create a components.json file, or check that you're in the correct directory.",
    })
...
export class ConfigParseError extends RegistryError {
  constructor(public readonly cwd: string, parseError: unknown) {
    ...
    super(message, {
      ...
      context: { cwd },
      suggestion:
        "Check your components.json file for syntax errors or invalid configuration. Run 'npx peace-ui@latest init' to regenerate a valid configuration.",
    })
...
export class RegistriesIndexParseError extends RegistryError {
  ...
  constructor(parseError: unknown) {
    ...
    super(message, {
      ...
      context: { parseError },
      suggestion:
        "The registries index may be corrupted or have invalid registry namespace format. Registry names must start with @ (e.g., @peace, @example).",
    })
