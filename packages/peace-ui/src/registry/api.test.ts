...
const server = setupServer(
  ...
  http.get(`${REGISTRY_URL}/styles/peace-v1/button.json`, () => {
    return HttpResponse.json({
      name: "button",
      type: "registry:ui",
      dependencies: ["@radix-ui/react-slot"],
  ...
  http.get(`${REGISTRY_URL}/registries.json`, () => {
    return HttpResponse.json([
      {
        name: "@peace",
        url: "https://peace-ui.dev/r/styles/{style}/{name}.json",
...
describe("getRegistryItem", () => {
  ...
  it("should include error code in RegistryNotFoundError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/non-existent.json`, () => {
  ...
  it("should include error code in RegistryUnauthorizedError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/protected.json`, () => {
  ...
  it("should include error code in RegistryForbiddenError", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/forbidden.json`, () => {
  ...
  it("should include error code in RegistryFetchError for 500 errors", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/server-error.json`, () => {
        return HttpResponse.json(
  ...
  it("should serialize error to JSON correctly", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/test-json.json`, () => {
        return HttpResponse.json({ error: "Not found" }, { status: 404 })
      })
    )
  ...
  it("should include timestamp in errors", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/timestamp-test.json`, () => {
        return HttpResponse.json({ error: "Not found" }, { status: 404 })
      })
    )
  ...
  it("should include response body in RegistryFetchError", async () => {
    ...
    server.use(
      http.get(`${REGISTRY_URL}/styles/peace-v1/bad-request.json`, () => {
...
describe("getRegistry", () => {
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryName = "@peace"
    const registryUrl = "https://peace-ui.dev/registry.json"
    ...
    const mockConfig = {
      ...
      registries: {
        "@peace": {
          url: "https://peace-ui.dev/{name}.json",
        },
      },
    } as any
...
describe("getRegistriesConfig", () => {
  ...
  it("should parse valid components.json with registries", async () => {
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "shadcn-test-"))
    const configFile = path.join(tempDir, "peace-ui.json")
  ...
  it("should handle complex registry configurations", async () => {
    ...
    const config = {
      style: "new-york",
      registries: {
        "@peace": "https://peace-ui.dev/r/styles/{style}/{name}.json",
    ...
    try {
      ...
      expect(result.registries?.["@peace"]).toBe(
        "https://peace-ui.dev/r/styles/{style}/{name}.json"
      )
