...
process.on("exit", (code) => {
  const filePath = path.resolve(process.cwd(), "peace-ui.json")
...
export const init = new Command()
  ...
  .option("--no-base-style", "do not install the base peace-ui style.")
  ...
  .action(async (components, opts) => {
    try {
      ...
      if (components.length > 0) {
        ...
        const componentsJsonPath = path.resolve(options.cwd, "peace-ui.json")
      ...
      deleteFileBackup(path.resolve(options.cwd, "peace-ui.json"))
      logger.break()
    } catch (error) {
      logger.break()
      handleError(error)
...
export async function runInit(
  ...
  const components = [
    // "index" is the default peace-ui style.
  ...
  const targetPath = path.resolve(options.cwd, "peace-ui.json")
...
async function promptForConfig(defaultConfig: Config | null = null) {
  ...
  return rawConfigSchema.parse({
    $schema: "https://peace-ui.dev/schema.json",
    style: options.style,
    tailwind: {
