...
const PEACE_UI_URL = REGISTRY_URL.replace(/\/r\/?$/, "")
...
export const create = new Command()
  .name("create")
  .description("create a new project with peace-ui")
  ...
  .action(async (name, opts) => {
    try {
      ...
      if (hasNoPreset) {
        ...
        const createUrl = getPeaceCreateUrl(
          Object.keys(searchParams).length > 0 ? searchParams : undefined
        )
        logger.log("Build your own peace-ui.")
        logger.log(
...
function buildInitUrl(preset: Preset, rtl: boolean) {
  ...
  return `${getPeaceInitUrl()}?${params.toString()}`
}
  ...
  if (presetArg === true) {
    ...
    const { selectedPreset } = await prompts({
      ...
      choices: [
        ...
        {
          title: "Custom",
          description: "Build your own on https://peace-ui.dev",
          value: "custom",
        },
      ],
    })
    ...
    if (selectedPreset === "custom") {
      const url = getPeaceCreateUrl(rtl ? { rtl: "true" } : undefined)
...
function getPeaceCreateUrl(searchParams?: Record<string, string>) {
  const url = new URL(`${PEACE_UI_URL}/create`)
  if (searchParams) {
...
function getPeaceInitUrl() {
  return `${PEACE_UI_URL}/init`
}
